  ...
  removePlace: (cityID, placeID) => {
    set((state) => ({
      countries: state.countries.map((country) => {
        const newCities = country.cities.map((city) => {
          if (city.id === cityID) {
            const places = city.places.filter((place) => place.id !== placeID);
            if (places.length === 0) {
              places.push({ id: uuidv4(), place: "", placePrice: "" });
            }
            return {
              ...city,
              places,
  ...
  removeFood: (cityID, foodID) => {
    set((state) => ({
      countries: state.countries.map((country) => {
        const newCities = country.cities.map((city) => {
          if (city.id === cityID) {
            const food = city.food.filter((meal) => meal.id !== foodID);
            if (food.length === 0) {
              food.push({ id: uuidv4(), foodName: "", foodPrice: "" });
            }
            return {
              ...city,
              food,
